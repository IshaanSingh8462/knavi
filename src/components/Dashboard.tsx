  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 text-left space-y-8">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <span className="text-xs uppercase tracking-widest text-primary-soft font-mono font-bold">
          Basecamp
        </span>
        <h1 className="font-serif font-black text-3xl sm:text-4xl text-ink mt-1">Your Weekly Standing</h1>
        <p className="text-sm text-ink-soft mt-1">Streaks, per-trail progress, and protected time at a glance.</p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <motion.div
          ...
          className="bg-surface border border-line p-6 rounded-2xl shadow-cozy flex items-start gap-4"
        >
          ...
          <div className="min-w-0">
            <span className="text-xs font-mono uppercase tracking-widest text-ink-soft block">Streak</span>
            <span className="text-3xl font-serif font-black text-ink">{streak.streak_count}d</span>
            <div className="mt-3 pt-3 border-t border-line flex items-center justify-between text-[10px] font-mono text-ink-soft uppercase">
              <span>Record</span>
        ...
        <motion.div
          ...
          className="bg-surface border border-line p-6 rounded-2xl shadow-cozy"
        >
          <span className="text-xs font-mono uppercase tracking-widest text-ink-soft block mb-3">Trail Progress</span>
          {hasActivePlan && branchStats.length > 0 ? (
              ...
              {branchStats.map((b) => (
                <div key={b.branch}>
                  <div className="flex items-center justify-between text-[11px] mb-1">
                    ...
                    <span className="font-mono text-ink-soft">{b.completed}/{b.total}</span>
                  </div>
            ...
            <p className="text-xs text-ink-soft italic">No active plan yet — design your week to get started.</p>
          )}
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-surface border border-line p-6 rounded-2xl shadow-cozy"
        >
          <span className="text-xs font-mono uppercase tracking-widest text-ink-soft block mb-2 flex items-center gap-1.5">
            <Shield className="w-3.5 h-3.5" /> Protected Time
          </span>
          {activities.length > 0 ? (
            <>
              <span className="text-3xl font-serif font-black text-moss">{protectedHours.toFixed(1)}h</span>
            ...
            <p className="text-xs text-ink-soft mt-1 leading-relaxed">
              Optional — add sports, lessons, or standing plans in setup and Strail will route around them.
            </p>
          )}
        </motion.div>
      </div>
      ...
      >
        {hasActivePlan ? (
          <button
            ...
            className="flex-1 py-4 bg-primary text-white font-sans font-bold hover:opacity-90 shadow-active rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-transform hover:-translate-y-0.5 text-sm"
          >
        ...
        {hasActivePlan && (
          <button
            onClick={onNavigateToSetup}
            className="py-4 px-6 bg-black/5 text-ink font-sans font-bold border border-line hover:bg-black/10 shadow-cozy rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all text-sm"
          >
            Rebuild Plan <Calendar className="w-4 h-4 text-ink-soft" />
          </button>
        )}
      </motion.div>
    </div>
